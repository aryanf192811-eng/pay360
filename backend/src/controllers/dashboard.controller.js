...
// (CLAUDE.md's Dynamic Data Mandate). All filters are optional.
...
async function getDashboard(req, res, next) {
  try {
    ...
    // Employee-scoping filters (department_id/employee_type) apply to attendance and time-off
    // too, not just payslips — PS: "Flexible filtering by Period and Department allows users to
    // analyze salary costs, ATTENDANCE, and LEAVE patterns" — joining to employees for both.
    const timeOffParams = [];
    ...
    // ── Payroll Alerts (unresolved warnings, grouped) ──
    ...
    // ── Department Overview (headcount + total salary, independent of period filter) ──
