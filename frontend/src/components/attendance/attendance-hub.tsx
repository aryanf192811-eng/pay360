...
export function AttendanceHub() {
  const { attendance, employees, schedules, addSchedule, addAttendanceRecord, updateAttendance } = useStore();
...
function AttendanceFormRecord({ selectedRecord, employees, setSelectedRecord, updateAttendance }: any) {
  const [isEditing, setIsEditing] = useState(false);
  const [editCheckIn, setEditCheckIn] = useState(selectedRecord.checkIn);
  const [editCheckOut, setEditCheckOut] = useState(selectedRecord.checkOut);
  const [editStatus, setEditStatus] = useState<any>(selectedRecord.status);

  const handleSave = () => {
    let hours = selectedRecord.workedHours;
    if (editCheckIn && editCheckOut) {
      const startHour = parseInt(editCheckIn.split(":")[0]);
      const endHour = parseInt(editCheckOut.split(":")[0]);
    ...
    <div className="flex-1 p-6 md:p-8 bg-white h-full flex flex-col mx-6 border border-slate-200 rounded-[24px] shadow-sm">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2">
          <button 
            ...
            className="w-10 h-10 bg-slate-50 border border-slate-200 rounded-full flex items-center justify-center text-slate-500 hover:text-[#714B67] hover:bg-white transition-all shadow-sm mr-2"
          >
      ...
      {isEditing ? (
        <div className="flex gap-3 mb-6">
          <button onClick={handleSave} className="px-6 py-2 bg-[#714B67] hover:bg-[#5a3a52] text-white rounded-xl text-sm font-bold shadow-sm transition-all">
            SAVE
          </button>
          <button onClick={() => setIsEditing(false)} className="px-6 py-2 border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all">
            CANCEL
          </button>
        </div>
      ) : (
        <button onClick={() => setIsEditing(true)} className="self-start mb-6 px-8 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-sm font-bold shadow-sm transition-all">
          EDIT
        </button>
      )}

      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 max-w-4xl bg-slate-50/50 border border-slate-100 p-8 rounded-2xl">
        <div className="space-y-6">
          <div className="flex flex-col">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Employee</label>
            <input type="text" readOnly value={employees.find((e: any) => e.id === selectedRecord.employeeId)?.name || selectedRecord.employeeId} className="w-full p-3 bg-slate-50 border border-slate-200/60 rounded-xl font-medium text-slate-900" />
          </div>
          <div className="flex flex-col">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Check In</label>
            {isEditing ? (
              <input type="time" value={editCheckIn} onChange={e => setEditCheckIn(e.target.value)} className="w-full p-3 bg-white border border-slate-300 rounded-xl font-medium text-slate-900 focus:ring-2 focus:ring-[#714B67]/30" />
            ) : (
              <input type="text" readOnly value={`${selectedRecord.date} ${selectedRecord.checkIn}`} className="w-full p-3 bg-slate-50 border border-slate-200/60 rounded-xl font-medium text-slate-900" />
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex flex-col">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Check Out</label>
            {isEditing ? (
              <input type="time" value={editCheckOut} onChange={e => setEditCheckOut(e.target.value)} className="w-full p-3 bg-white border border-slate-300 rounded-xl font-medium text-slate-900 focus:ring-2 focus:ring-[#714B67]/30" />
            ) : (
              <input type="text" readOnly value={`${selectedRecord.date} ${selectedRecord.checkOut}`} className="w-full p-3 bg-slate-50 border border-slate-200/60 rounded-xl font-medium text-slate-900" />
            )}
          </div>
          <div className="flex flex-col">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Worked Hours</label>
            <input type="text" readOnly value={`${selectedRecord.workedHours} Hours`} className="w-full p-3 bg-slate-50 border border-slate-200/60 rounded-xl font-black text-[#00A09D]" />
          </div>
          
          {isEditing && (
            <div className="flex flex-col">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Status</label>
              <select value={editStatus} onChange={e => setEditStatus(e.target.value as any)} className="w-full p-3 bg-white border border-slate-300 rounded-xl font-medium text-slate-900 focus:ring-2 focus:ring-[#714B67]/30">
                <option value="Present">Present</option>
